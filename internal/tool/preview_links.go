...
Device preview is enabled per workspace and per platform. PermissionDenied means the workspace does not have it yet — tell the user to ask Bitrise support, do not retry.`),
	...
	Handler: func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if !hasDevice {
			return mcp.NewToolResultError("device_spec is required — a preview link always names the device to boot (platform \"ios\" or \"android\")"), nil
		}
		...
		if machineType := request.GetString("machine_type", ""); machineType != "" {
