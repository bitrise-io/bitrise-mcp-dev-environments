...
Boots an iOS simulator (platform "ios") or Android emulator (platform "android") alongside the session and streams it — ready for adb / xcrun simctl / serve-sim; optionally a human can watch and drive it on its page in the RDE web UI — device.page_url in the response, for a browser where the user is logged in to Bitrise. Zero-config rule: OMIT BOTH stack_id and machine_type (and cluster) — the deployment's known-good per-platform defaults apply; giving exactly one of the two is rejected. Name them only when you must (the guide says which stacks fit); with a template, its stack and machine type are used and must fit. Optionally pass artifact to pre-install an app build. Delete the session when done.
